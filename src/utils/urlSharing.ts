...
export async function createShareUrl(
	...
	try {
		...
		// Use query parameter routing for GitHub Pages compatibility
		const shareUrl = `${baseUrl}?page=share&s=${encryptedShare}&iv=${ivString}&salt=${saltString}`;
		...
		return {
			...
			error:
				error instanceof Error
					? error
					: new Error(`Failed to create share URL: ${String(error)}`, {
							cause: error,
						}),
		};
	}
 ...
 */
export async function decryptShareFromUrl(
	params: DecryptShareUrlParams,
): Promise<Result<string>> {
	try {
		...
		return {
			...
			error:
				error instanceof Error
					? error
					: new Error("Invalid password or corrupted share data"),
		};
	}
}
